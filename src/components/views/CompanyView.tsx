...
const CompanyView = ({
    ...
    // Get all projects from the specified company
    const allProjects = projects;
    const companyProjects = allProjects.filter(project => {
    // Extract company name from artist field (e.g., "Software Engineer - DDx" -> "DDx")
        const artistParts = project.artist?.split(' - ');
        const projectCompany = artistParts?.[artistParts.length - 1];
        ...
        return (
            <div className="text-white p-4 md:p-6">
                <div className="text-center py-12">
                    ...
                    <p className="text-gray-400">No projects found for company &quot;{company}&quot;</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="text-white p-4 md:p-6">
            <div className="flex flex-col md:flex-row md:items-end space-y-4 md:space-y-0 md:space-x-6 mb-6 md:mb-8">
                <div className="w-48 h-48 md:w-64 md:h-64 mx-auto md:mx-0 bg-gradient-to-br from-blue-500 to-cyan-500 rounded-lg flex items-center justify-center shadow-2xl">
                ...
                <div className="text-center md:text-left">
                    ...
                    <h1 className="text-3xl md:text-5xl lg:text-6xl font-bold mb-2 md:mb-4">{company}</h1>
                    <p className="text-gray-400">Josh Dutcher • {companyProjects.length} projects</p>
                </div>
            </div>
            ...
            <div className="flex items-center space-x-4 md:space-x-6 mb-6 md:mb-8">
                <button 
                    ...
                    onClick={() => companyProjects[0] && onPlayProject(companyProjects[0], { projects: companyProjects })}
        >
                ...
                {companyProjects.map((project, _index) => (
                    <div key={project.id} className="grid grid-cols-12 gap-4 items-center py-2 hover:bg-gray-800 rounded-lg px-2 group">
                        <div className="col-span-1 text-gray-400 text-sm">
                            ...
                            <Play className="w-4 h-4 hidden group-hover:block cursor-pointer" fill="currentColor" onClick={() => onPlayProject(project, { projects: companyProjects })} />
                        </div>
