...
- [x] **Spotify-style synced lyrics** - Lines highlight white as they're sung, others stay gray, view auto-scrolls

**Implementation**:
- Synced lyrics stored as JSON files in `src/data/lyrics/` with `{ time, text }` entries
- Stanza breaks encoded as `{ time, text: "" }` entries
- Instrumental breaks use 🎵 emoji with pulse animation when active
- `LyricsPreview` component in NowPlaying sidebar with scroll-up animation
- Desktop `LyricsView` with auto-scroll to active line (1/3 from top)
- Mobile `MobilePlayerView` with synced preview and full-screen expanded view
- Left/right arrow keyboard seek (±5s)
- Previous track button restarts song if past 5 seconds
- `displayLyrics` kept in project files as reference only (not used for rendering)
- Timestamps created using [LRC Maker](https://github.com/magic-akari/lrc-maker)

**Data pipeline**: MP3 → LRC Maker (manual tap-to-sync) → LRC → conversion script → JSON
